function [D PD] = allfitdist_woFig(data,sortby,varargin)
    ...
    if ~discind
      ...
      %  subplot(2,1,1)
        if lgx
            ...
            plot(xi,fi,'k',xi,ys,'LineWidth',1.5)
        end
        legend(['empirical',{D(inds).DistName}],'Location','NW')
        xlabel('Relative Difference x 100 [%]');
