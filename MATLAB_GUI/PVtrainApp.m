function [tree] = PVtrainApp(Location,Pvsystem,t,UTC,WeatherforecastOrg, DiffMethod, Method,server,Temp,TempEff,Zen,ZenEff,ZenDir,OhmEff)
%% time inputs
Starttime=t(1); %set Start time as first element of vector t
Endtime=t(end); %set Start time as first element of vector t
timestep=60;    %1-hr resolution


%% Data aquiration (Clear Sky + AC Power)
PVClearsky=clearskygenApp(Pvsystem,Location,t,UTC,Zen,ZenEff,ZenDir,OhmEff);

AC_power=emoncmsApp(server,Starttime,Endtime,timestep);
